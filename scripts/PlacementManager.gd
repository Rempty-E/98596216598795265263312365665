...
func _place_building(index: int):
	...
	# ДОБАВЬ ЭТО:
	print("--- ПРОВЕРКА ЗДАНИЯ ---")
	print("Имя: ", sprite.name)
	print("Путь к текстуре: ", sprite.texture.resource_path)
	print("Размер текстуры: ", sprite.texture.get_size())
	print("Масштаб спрайта: ", sprite.scale)
	print("Регион включен: ", sprite.region_enabled)
